...
class Trader:
    ...
    def decide_order(self, r_hat, best_bid, best_ask, timestamp):
        if(timestamp%2 == 0) and (self.is_fast == False) and (self.stored_order != None):
            order_to_be_placed=self.stored_order
            self.stored_order = None
            return order_to_be_placed
        ...
        self.stored_order = (f"{side}_limit", limit_price)
        return None
        # return (f"{side}_limit", limit_price)
