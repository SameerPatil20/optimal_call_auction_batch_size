...
lambda_s = 0.002      # slow traders' reentry rate
...
market_type = 'CDA'
call_interval = 100  # experiment with 100, 500, 1000, etc.
...
for t in range(1, T+1):
    ...
    for i in arrivals:
        ...
        if isinstance(market, MarketCDA):
            otype, price = tr.decide_order(r_hat, best_bid, best_ask)
            market.time=t
            ...
            otype,price=tr.decide_order(r_hat,None,None)
            market.add_order(tr, otype, price); tr.outstanding_order=(otype,price)
    # CALL clearing
